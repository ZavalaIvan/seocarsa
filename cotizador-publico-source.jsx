...
const bP=(e,ap,rnGross,mx,plComp,isr,sa,blMensual)=>{
  ...
  for(let y=1;y<=(mx-e);y++){
    ...
    let netoOp1=0,netoOp2=0,retOp1=0,retOp2=0,esExento65=false;
    if(isr==='151'){
      if(edadR>=65&&fin){
        esExento65=true;
        // Opción 1: Pago único — exento hasta 90 UMAs anuales
        const exento1=umaAnual*90;
        const gravado1=Math.max(0,afterRet-exento1);
        retOp1=Math.round(gravado1*0.20);
        netoOp1=afterRet-retOp1;
        // Opción 2: Retiros anuales (10 años) — exento 15 UMAs/año
        const retiroAnual=Math.round(afterRet/10);
        let totRet2=0;
        for(let ra=0;ra<10;ra++){
          const umaRa=umaAnual*Math.pow(1+INF,ra);
          const exRa=umaRa*15;
          const grRa=Math.max(0,retiroAnual-exRa);
          totRet2+=Math.round(grRa*0.20);
        }
        retOp2=totRet2;
        netoOp2=afterRet-retOp2;
        ret=retOp1;
      } else {
        ret=Math.round(afterRet*0.20);
        retOp1=ret;retOp2=ret;netoOp1=afterRet-ret;netoOp2=afterRet-ret;
      }
    } else {
      if(edadR>=60&&y>=5) ret=0;
    ...
    d.push({edad:e+y,y,s:sTotal,fund:Math.round(fi+fr),bono,apY:Math.round(apM*12),blY:Math.round(blY),acTot:Math.round(acTot),disp,dispNeto:dN,retencion:ret,cargoRetiro:Math.round(disp*cRet),dispPct,esFinalPlazo:fin,bl:blSA,sbl:sTotal+blSA,esExento65,netoOp1,netoOp2,retOp1,retOp2});
  }
  return d;
};
...
.Lg span{color:var(--gold);font-style:italic;font-weight:400}
...
.Inp:focus{border-color:var(--earth);box-shadow:0 0 0 4px rgba(0,55,129,.1)}
...
.Cc:hover{border-color:var(--earth-light);transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,55,129,.08)}
...
.Nb-next{background:#003781;color:#fff;flex:2;box-shadow:0 4px 16px rgba(0,55,129,.25)}
.Nb-next:hover{background:#1565C0;transform:translateY(-1px);box-shadow:0 6px 22px rgba(0,55,129,.35)}
.Nb-next:disabled{background:#9BB3CF;cursor:not-allowed;transform:none;box-shadow:none}
...
.R-main{margin:0 20px 16px;background:linear-gradient(135deg,#003781 0%,#1565C0 100%);border-radius:26px;padding:32px 24px;color:#fff;position:relative;overflow:hidden}
...
/* ISR OPTIONS CARD */
.R-isr-opts{margin:0 20px 16px;background:#fff;border-radius:22px;padding:22px 20px;border:1px solid var(--cream-dark)}
.R-isr-opts-t{font-family:'Fraunces',serif;font-size:17px;font-weight:600;color:var(--ink);margin-bottom:6px;letter-spacing:-0.3px}
.R-isr-opts-d{font-size:12px;color:var(--text-muted);line-height:1.55;margin-bottom:16px}
.R-isr-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.R-isr-card{border-radius:14px;padding:16px 14px;position:relative}
.R-isr-card-tag{font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:8px;opacity:.7}
.R-isr-card-label{font-size:12px;color:var(--text-muted);margin-bottom:4px}
.R-isr-card-val{font-family:'Fraunces',serif;font-size:20px;font-weight:700;margin-bottom:2px}
.R-isr-card-sub{font-size:10px;opacity:.6}
.R-isr-card.op1{background:rgba(0,55,129,.06);border:1px solid rgba(0,55,129,.15)}
.R-isr-card.op1 .R-isr-card-val{color:var(--sage)}
.R-isr-card.op2{background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.2)}
.R-isr-card.op2 .R-isr-card-val{color:var(--gold-dark)}
.R-isr-foot{font-size:10px;color:var(--text-muted);margin-top:14px;line-height:1.55;font-style:italic}
...
.R-retiro-fin{background:rgba(0,55,129,.05)}
...
.R-cta{margin:20px;padding:24px 20px;background:linear-gradient(135deg,#0B1D3A 0%,#003781 100%);border-radius:24px;text-align:center;color:#fff;position:relative;overflow:hidden}
...
.R-cta-h::after{content:' Solicita tu propuesta formal por WhatsApp al 999 296 8025';font-size:13px;font-weight:400;display:block;margin-top:6px}
...
function App(){
  ...
  const [apRaw,setApRaw]=useState('');
  const [apFocused,setApFocused]=useState(false);
          ...
          <div className="Step">
            ...
            <div className="Fld">
              ...
              <div style={{position:'relative'}}>
                ...
                <input className="Inp" style={{paddingLeft:32}} inputMode="numeric"
                  value={apFocused?apRaw:fmt(f.ap)}
                  onFocus={()=>{setApRaw(String(f.ap));setApFocused(true);}}
                  onChange={e=>{setApRaw(e.target.value.replace(/[^0-9]/g,''));}}
                  onBlur={()=>{const n=parseInt(apRaw)||apLimits.min;const v=Math.min(apLimits.max,Math.max(apLimits.min,n));u('ap',v);setApFocused(false);}}
                />
              </div>
  ...
  const waMsg=encodeURIComponent(
    `Hola, quiero información sobre Ahorro / Retiro.\n`+
    `Vengo desde: Página web\n`+
    `Página: Cotizador público de ahorro\n`+
    `Botón: Pedir mi propuesta por WhatsApp\n\n`+
    ...
    (hasProteccion?
      ...
      :
      `Protección: Requiere asesoría personalizada por mi rango de edad.\n\n`+
      `Aportación mensual ahorro: ${fm(f.ap)}\n\n`
    )+
    `Agradezco me envíen la propuesta formal detallada.`
  );
  const waLink=`https://wa.me/529992968025?text=${waMsg}`;
  ...
  return(
    <div className="App Rw"><style>{CSS}</style>
      <div className="Fc">
        ...
        {f.isr==='151'&&(()=>{
          // Find the row at age 65 — either from extended projection or main plan
          let row65 = null;
          if(R.proj65Rows&&R.proj65Rows.length>0){
            row65 = R.proj65Rows[R.proj65Rows.length-1];
          } else if(last?.esExento65){
            row65 = last;
          }
          if(!row65?.esExento65) return null;
          const titulo = R.projExtended ? 'Si continúas hasta los 65 — Opciones fiscales' : 'Opciones fiscales al retirar (Art. 151)';
          return <div className="R-isr-opts">
            <h3 className="R-isr-opts-t">💰 {titulo}</h3>
            <p className="R-isr-opts-d">Al cumplir 65 años puedes elegir cómo recibir tu fondo:</p>
            <div className="R-isr-grid">
              <div className="R-isr-card op1">
                <div className="R-isr-card-tag">Opción 1</div>
                <div className="R-isr-card-label">Pago único</div>
                <div className="R-isr-card-val">{fm(row65.netoOp1)}</div>
                <div className="R-isr-card-sub">Exento hasta 90 UMAs anuales</div>
                {row65.retOp1>0&&<div className="R-isr-card-sub">ISR sobre excedente: {fm(row65.retOp1)}</div>}
                {row65.retOp1===0&&<div className="R-isr-card-sub" style={{color:'var(--sage)',fontWeight:600}}>100% exento</div>}
              </div>
              <div className="R-isr-card op2">
                <div className="R-isr-card-tag">Opción 2</div>
                <div className="R-isr-card-label">Retiros anuales (10 años)</div>
                <div className="R-isr-card-val">{fm(row65.netoOp2)}</div>
                <div className="R-isr-card-sub">Exento hasta 15 UMAs por año</div>
                {row65.retOp2>0&&<div className="R-isr-card-sub">ISR total estimado: {fm(row65.retOp2)}</div>}
                {row65.retOp2===0&&<div className="R-isr-card-sub" style={{color:'var(--gold-dark)',fontWeight:600}}>100% exento</div>}
              </div>
            </div>
            <p className="R-isr-foot">Valores estimados con UMA 2026 ($113.14/día) proyectada con inflación del 4%. El tratamiento fiscal definitivo depende de la legislación vigente al momento del retiro.</p>
          </div>;
        })()}
            ...
            <AreaChart data={R.proj} margin={{top:8,right:14,left:0,bottom:4}}>
              <defs>
                ...
                <linearGradient id="gShield" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#003781" stopOpacity={.28}/>
                  <stop offset="100%" stopColor="#003781" stopOpacity={.02}/>
                </linearGradient>
              </defs>
              ...
              {hasProteccion&&<Area type="monotone" dataKey="bl" stroke="#003781" fill="url(#gShield)" strokeWidth={2} name="Protección a tu familia"/>}
        ...
        {showDetails&&<div className="R-det">
          <div className="R-pd">
            ...
            <div className="R-pd-g">
              ...
              <div className="R-pd-b" style={{background:f.isr==='151'?'rgba(0,55,129,.08)':'var(--cream)'}}><div className="R-pd-n" style={{fontSize:13}}>{f.isr==='151'?'Deducible':'Flexible'}</div><div className="R-pd-l">Tipo de plan</div></div>
            </div>
          </div>
